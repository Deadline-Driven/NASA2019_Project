...
real_output_y = np.zeros((1, 1, 144, 192))
for i in range(144):
    for j in range(192):
        if i%2==0 and j%2==0:
            real_output_y[0][0][i][j] = img_out_y[0][0][i//2][j//2]
        if i%2==0 and j%2==1:
            real_output_y[0][0][i][j] = img_out_y[0][1][i//2][(j-1)//2]
        if i%2==1 and j%2==0:
            real_output_y[0][0][i][j] = img_out_y[0][2][(i-1)//2][j//2]
        if i%2==1 and j%2==1:
            real_output_y[0][0][i][j] = img_out_y[0][3][(i-1)//2][(j-1)//2]


cv2_out_img_y = np.uint8((real_output_y[0] * 255.0).clip(0, 255)[0])
