...
# 1) load dataset
# ***
data_file_name = 'NASA_Data.csv'
df = read_csv(data_file_name, engine='python', skipfooter=3)

dft = df[['Boom 2 Wind counter 7', 'Boom 2 Wind counter 2',
          'Boom 2 Wind counter 3', 'Boom 2 Wind counter 4',
          'Boom 2 Wind counter 5', 'Boom 2 Wind counter 6',
          'Boom 2 Wind counter 8']]


def missing_func(test_col, missing_rate=0.1, block_missing=False):
    random_vec = np.random.rand(test_col.shape[0], 1)
    evg = test_col[0]
    if block_missing:
        # Block missing vector
        binary_vec = np.round(random_vec + 0.5 - missing_rate / 3)
        ign_flag = False
        for num in range(len(binary_vec)):

            if binary_vec[num] == 0 and (not ign_flag):
                binary_vec[num - 1] = 0
                binary_vec[num + 1] = 0
                ign_flag = True
            else:
        ...
        # Random missing vector
        binary_vec = np.round(random_vec + 0.5 - missing_rate)

    missing_col = np.multiply(test_col, binary_vec)
    for num in range(len(missing_col)):
        evg = (evg + test_col[num]) / 2
        if missing_col[num] == 0:
            missing_col[num] = evg
...
testplot1 = missing_func(dataset1[lead_time:,target_col], missing_rate=0.1,
                         block_missing=False).reshape(-1, 1)
...
if process == 'Y' or 'y':
    ...
    dataplot = scalerY.fit_transform(testplot1)
...
if file_write == "y":
    ...
    filename = 'Boom 2 Wind counter 7_fill'+ str(n_epochs) + str(lead_time) + '_' + stamp + '.xlsx'    #example output file
